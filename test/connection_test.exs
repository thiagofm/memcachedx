defmodule Memcachedx.ConnectionTest do
  ...
  test 'add from doc example' do
    {:ok, pid} = Connection.start_link([hostname: "localhost", port: 11211])
    assert Connection.run(pid, [:add, [key: "Hello", value: "World", cas: 0, flags: 0xdeadbeef, expiry: 0x00000e10]]) == {:ok, [opcode: :add, key_length: 0, extras_length: 0, total_body_length: 0, opaque: 0, cas: 1]}
  end

  test 'add' do
    {:ok, pid} = Connection.start_link([hostname: "localhost", port: 11211])
    assert Connection.run(pid, [:add, [key: "Hello", value: "World", flags: 0, expiry: 0]]) == {:ok, [opcode: :add, key_length: 0, extras_length: 0, total_body_length: 0, opaque: 0, cas: 1]}
  end

  test 'set from doc example' do
    {:ok, pid} = Connection.start_link([hostname: "localhost", port: 11211])
    assert Connection.run(pid, [:set, [key: "Hello", value: "World", flags: 0, expiry: 0]]) == {:ok, [opcode: :set, key_length: 0, extras_length: 0, total_body_length: 0, opaque: 0, cas: 1]}
  end
  ...
  test 'delete success' do
    ...
    {:ok, pid} = Connection.start_link([hostname: "localhost", port: 11211])
    assert Connection.run(pid, [:delete, [key: "Hello"]]) == {:ok, [opcode: :delete, key_length: 0, extras_length: 0, total_body_length: 0, opaque: 0, cas: 0]}
  end

  test 'delete fail' do
    {:ok, pid} = Connection.start_link([hostname: "localhost", port: 11211])
    assert Connection.run(pid, [:delete, [key: "Hello"]]) == {:error, [opcode: :delete, key_length: 0, extras_length: 0, total_body_length: 9, opaque: 0, cas: 0, value: "Not found"]}
  end
  ...
  test 'incr success' do
    ...
    {:ok, pid} = Connection.start_link([hostname: "localhost", port: 11211])
    assert Connection.run(pid, [:incr, [key: "counter", delta: 0x01, initial: 0x00, expiration: 0x00000e10]]) == {:ok, [opcode: :incr, key_length: 0, extras_length: 0, total_body_length: 8, opaque: 0, cas: 2, value: <<0, 0, 0, 0, 0, 0, 0, 2>>]}
    end

  test 'incr when key doesn\'t exist' do
    {:ok, pid} = Connection.start_link([hostname: "localhost", port: 11211])
    assert Connection.run(pid, [:incr, [key: "counter3", delta: 0x01, initial: 0x03, expiration: 0x00000e10]]) == {:ok, [opcode: :incr, key_length: 0, extras_length: 0, total_body_length: 8, opaque: 0, cas: 1, value: <<0, 0, 0, 0, 0, 0, 0, 3>>]}
  end
  ...
  test 'get doc example' do
    ...
    {:ok, pid} = Connection.start_link([hostname: "localhost", port: 11211])
    assert Connection.run(pid, [:get, [key: "Hello"]]) == {:ok, [opcode: :get, key_length: 0, extras_length: 4, total_body_length: 9, opaque: 0, cas: 1, extras: 0, value: "World"]}
  end
