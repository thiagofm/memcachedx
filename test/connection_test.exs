defmodule Memcachedx.ConnectionTest do
  ...
  test 'delete fail' do
    {:ok, pid} = Memcachedx.Connection.start_link([hostname: "localhost", port: 11211])
    assert Memcachedx.Connection.run(pid, [:delete, [key: "Hello"]]) == {:error, [cas: 0, opcode: :delete, total_body: 9]}
  end
  ...
  test 'incr success' do
    ...
    {:ok, pid} = Memcachedx.Connection.start_link([hostname: "localhost", port: 11211])
    assert Memcachedx.Connection.run(pid, [:incr, [key: "counter", delta: 0x01, initial: 0x00, expiration: 0x00000e10]]) == {:ok, [cas: 2, opcode: :incr, total_body: 8]}
  end

  test 'incr when key doesn\'t exist' do
    {:ok, pid} = Memcachedx.Connection.start_link([hostname: "localhost", port: 11211])
    assert Memcachedx.Connection.run(pid, [:incr, [key: "counter3", delta: 0x01, initial: 0x03, expiration: 0x00000e10]]) == {:ok, [cas: 2, value: 0x03, total_body: 8, opcode: :incr]}
  end
end
